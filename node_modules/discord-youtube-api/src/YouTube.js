...
class YouTube {
	...
	async searchVideos(query) {
		...
		const type = "video";
		try {
			/*encodeURI ensures we are able to query foreign characters like '星空☆ディスティネーション' */
			const response = await snekfetch.get(encodeURI(`${this.base}/search?part=${part}&key=${this.key}&maxResults=${max}&type=${type}&q=${query}`));
	...
	/* Modified to fetch 50 videos from a playlist */
	async getPlaylistByID(id) {
		...
		try {
			...
			/* Paging */
			const response = await snekfetch.get(encodeURI(`${this.base}/playlistItems?part=${part}&key=${this.key}&playlistId=${id}&maxResults=${max}`));
				var videos = await Promise.all(JSON.parse(response.text).items.map(async item => {
					try {
						return await this.getVideoByID(item.snippet.resourceId.videoId); // 'var videos' catches this return statement.
					} catch (err) {
						return null;
					}
				}));
